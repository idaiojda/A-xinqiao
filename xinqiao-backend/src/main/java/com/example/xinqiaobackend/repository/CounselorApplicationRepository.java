...
public interface CounselorApplicationRepository extends JpaRepository<CounselorApplication, Long> {
    ...
    @Query("select a from CounselorApplication a where (:status is null or a.status = :status) and (:q is null or a.realName like concat('%', :q, '%') or cast(a.userId as string) like concat('%', :q, '%')) order by a.createdAt desc")
