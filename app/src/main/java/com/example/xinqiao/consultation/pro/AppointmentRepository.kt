...
class AppointmentRepository(private val context: Context) {
    ...
    fun fetchSlots(consultantId: String, date: String, token: String?): Result<List<SlotTime>> {
    ...
    fun submitAppointment(req: AppointmentRequest, token: String?): Result<String> {
            ...
            client.newCall(builder.build()).execute().use { resp ->
                if (!resp.isSuccessful) {
                    ...
                    val id = when {
                        ...
                        obj.has("data") -> obj.optJSONObject("data")?.optString("id")
                        else -> ""
                    }
