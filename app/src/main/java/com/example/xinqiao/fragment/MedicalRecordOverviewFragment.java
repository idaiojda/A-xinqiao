...
import android.graphics.Color;
import com.example.xinqiao.widget.RichEmotionMarkerView;
...
public class MedicalRecordOverviewFragment extends Fragment {
    ...
    private void renderEmotionTrend(LineChart chart, List<EmotionDiaryEntity> diaries, int days) {
        ...
        chart.setMarker(new RichEmotionMarkerView(requireContext(), labels, dayCount, dayNote));
        chart.animateX(600);
        chart.invalidate();
    }
    ...
    private void initActions(View view) {
        ...
        if (btnOpenTestRecords != null) {
            btnOpenTestRecords.setOnClickListener(v -> {
                // 跳转到主界面并直接打开测试记录（通过深链参数）
                Intent intent = new Intent(getContext(), MainActivity.class);
                intent.putExtra("open", "test_records");
                startActivity(intent);
            });
        }
        ...
        if (btnOpenAI != null) {
            btnOpenAI.setOnClickListener(v -> {
                // 跳转到主界面并打开“咨询”聚合页（含AI浮窗）
                Intent intent = new Intent(getContext(), MainActivity.class);
                intent.putExtra("open", "consultation");
                startActivity(intent);
            });
        }
